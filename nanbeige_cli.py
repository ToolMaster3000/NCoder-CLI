...
def parse_args():
    ...
    p.add_argument("--max-tokens", type=int, default=2048,
                    help="Max tokens for a generated reply. Raised from 1024 for this "
                         "tool's actual use case — a completely ordinary ~100-line script "
                         "runs close to 900 tokens once written out, so 1024 left almost no "
                         "room for any explanation alongside it. 2048 comfortably fits a "
                         "real script + a short explanation while still leaving most of the "
                         "default 6144-token context window (--ctx-size) for conversation "
...
def _gbnf_object(required_fields, optional_fields=(), rule_prefix=None):
    """Build a GBNF object rule body. required_fields is [(key, ref), ...]
    that must always be present, in order. optional_fields is the same
    shape but each field may be entirely omitted — modeled as a chain of
    nested optional groups (built backward from the last field), rather
    than forcing every property in a tool's schema to always be present
    regardless of whether it's actually in that tool's JSON-schema
    "required" list. That forcing was a real bug: pop_subtask's
    verified_by is explicitly optional ("leave empty only if there was
    genuinely nothing to verify"), but being coerced into filling it on
    every single call meant it almost always came back as an empty
    string even when the model had no organic reason to think about
    verification yet — which meant _needs_verification's bounce-once
    gate fired on nearly every qualifying subtask's first attempt,
    not because the keyword match was wrong, but because verified_by
    could never come back meaningfully filled in on a first try.

    Optional fields, if present, must appear as a prefix run in schema
    order — the model can supply the first K of them and skip the rest,
    but can't skip one in the middle while supplying a later one. That's
    a standard, reasonable simplification for grammar-constrained JSON
    (matches how llama.cpp's own json-schema-to-grammar handles this),
    and it's a large improvement over every optional field always being
    mandatory.

    rule_prefix must be given (and unique per call site) if
    optional_fields is non-empty, to name the helper tail rules this
    defines. Returns (object_body, extra_rule_lines) — the caller must
    splice extra_rule_lines into its own output alongside object_body.
    """
    extra_lines = []
    tail_refs = [None] * (len(optional_fields) + 1)
    for i in range(len(optional_fields) - 1, -1, -1):
        key, value_ref = optional_fields[i]
        this_tail = f'{rule_prefix}-opt{i}'
        inner = f' {tail_refs[i + 1]}' if tail_refs[i + 1] else ''
        # A leading comma is needed unless this could be the very first
        # field written into the object — i.e. it's optional-field index
        # 0 AND there are no required fields ahead of it.
        needs_comma = i > 0 or bool(required_fields)
        comma = '"," ws ' if needs_comma else ''
        extra_lines.append(
            f'{this_tail} ::= ( {comma}"\\"{key}\\"" ws ":" ws {value_ref}{inner} )?'
        )
        tail_refs[i] = this_tail

    parts = ['"{" ws']
    if required_fields:
        for i, (key, value_ref) in enumerate(required_fields):
            sep = ' "," ws ' if i > 0 else ' '
            parts.append(f'{sep}"\\"{key}\\"" ws ":" ws {value_ref}')
        if tail_refs[0]:
            parts.append(f' {tail_refs[0]}')
    elif tail_refs[0]:
        parts.append(f' {tail_refs[0]}')
    parts.append(' ws "}"')
    ...
    for t in tools:
        ...
        for pname, pdef in props.items():
            if "enum" in pdef:
                ...
                field = (pname, "string")
            (required_fields if pname in required_names else optional_fields).append(field)

        args_rule = f'args-{rule_id}'
        args_body, args_extra = _gbnf_object(required_fields, optional_fields, rule_prefix=args_rule)
        lines.extend(args_extra)
        lines.append(f'{args_rule} ::= {args_body}')
        call_body, call_extra = _gbnf_object([("tool", _gbnf_literal(name)),
                                               ("arguments", args_rule)])
        lines.extend(call_extra)
        lines.append(f't-{rule_id} ::= {call_body}')
    return lines
...
def chat_completion(base_url, messages, max_tokens, temperature, grammar=None, disable_thinking=False):
    ...
    disable_thinking: Nanbeige4.1-3B's chat template defaults to
    enable_thinking=true, so every response — including these short
    internal helper calls — starts with a <think>...</think> block
    before the actual answer. That's fine (even desirable) for the
    real user-facing reply, but calls like classify_task_complexity
    that expect one bare word back within a small token budget need
    it off, or the budget gets consumed by the reasoning preamble
    before the real answer is ever reached. Grammar-constrained calls
    don't need this — the grammar already forces the first token to
    conform to JSON, which rules out a <think> preamble structurally."""
    payload = {
        "messages": messages,
        "max_tokens": max_tokens,
        "temperature": temperature,
    ...
    if not resp.ok:
        # llama-server puts the actual reason (e.g. "failed to parse
        # grammar", "context size exceeded") in the response body —
        # resp.raise_for_status() alone discards that, leaving nothing
        # to diagnose a 4xx/5xx with beyond the status code.
        try:
            detail = resp.json().get("error", {}).get("message") or resp.text
        except (ValueError, AttributeError):
            detail = resp.text
        raise requests.exceptions.HTTPError(
            f"{resp.status_code} from llama-server: {detail[:500]}", response=resp,
        )
...
def classify_task_complexity(base_url, original_goal, max_tokens):
    ...
    try:
        msg = chat_completion(
            base_url, classify_prompt, min(max_tokens, 20), 0.0,
            grammar=CLASSIFY_GRAMMAR, disable_thinking=True,
        )
...
def check_decomposable(base_url, goal, max_tokens):
    ...
    try:
        msg = chat_completion(
            base_url, check_prompt, min(max_tokens, 60), 0.0,
    ...
    # Match the whole first word, not just a prefix — verdict.startswith("NO")
    # would misread "Nonetheless, yes this is decomposable" or "Notably,
    # this has several concrete steps" as a firm NO, since both happen to
    # start with the two letters "No". That's exactly backwards from this
    # function's own stated design: a false NO incorrectly blocks a
    # legitimate task from ever starting, which is the worse failure mode.
    first_word_match = re.match(r'\s*([A-Za-z]+)', verdict)
    first_word = first_word_match.group(1).upper() if first_word_match else ""
    if first_word == "NO":
        reason = verdict[first_word_match.end():].strip(" :-—,") or "the goal is too vague to break into concrete steps"
        return False, reason
    return True, ""
...
# Word-boundary-anchored at the start only (not the end), so inflected
# forms still match — "testing"/"tested", "fixes"/"fixed", "debugging",
# "coder", "facts", "corrected", etc. all still count. What a plain `kw
# in goal_lower` substring check got wrong: with no boundary at all, it
# also matched these keywords buried inside unrelated common words —
# "test" inside "latest"/"fastest"/"greatest", "fact" inside
# "manufacture"/"satisfaction", "patch" inside "dispatch", "code" inside
# "encode"/"decode", "fix" inside "prefix"/"suffix" — which is why the
# gate was firing on almost everything. Anchoring the start only (no \b
# at the end) rules those out, since none of them start a word with the
# bare keyword, while still catching every inflected form of the real
# keyword.
_VERIFICATION_RE = re.compile(
    r'\b(?:' + '|'.join(re.escape(kw) for kw in _VERIFICATION_KEYWORDS) + r')',
    re.IGNORECASE,
)
...
def _needs_verification(goal):
    ...
    return bool(_VERIFICATION_RE.search(goal or ""))
...
def detect_contradiction(base_url, new_name, new_result, completed_results, max_tokens):
    ...
    try:
        msg = chat_completion(base_url, check_prompt, min(max_tokens, 60), 0.0, disable_thinking=True)
        verdict = (msg.get("content") or "").strip()
    except requests.exceptions.RequestException:
        return ""

    # Whole-first-word match, not a prefix — "Nonetheless, there IS a
    # conflict..." starts with the exact 4 letters "NONE" too, which
    # would silently swallow a real, important contradiction warning.
    first_word_match = re.match(r'\s*([A-Za-z]+)', verdict)
    first_word = first_word_match.group(1).upper() if first_word_match else ""
    if first_word == "NONE":
        return ""
...
def get_plan_and_decision(base_url, messages, max_tokens, temperature):
    ...
    the plan text AND the tool-call decision, replacing what used to be
    two separate calls (an unconstrained plan call, then a separate
    grammar-constrained decision call) with one. Falls back to an
    empty plan and no tool calls on malformed output rather than
    raising — a malformed round should skip cleanly (caught by the
    existing "no calls -> break" logic) rather than crash the whole turn."""
    prompt = messages + [{
        "role": "user",
        "content": (
    ...
    try:
        msg = chat_completion(
            # Was capped at 350 with no stated rationale for that specific
            # number — fine for a short plan sentence plus a simple call
            # (a web_search query, a file path), but this same generation
            # also has to fit the ENTIRE argument content for calls like
            # write_file inline as one JSON string. A completely ordinary
            # ~100-line script needs roughly 900 tokens once JSON-escaped,
            # so 350 meant generation reliably got cut off mid-string on
            # any non-trivial file write, producing invalid/truncated
            # JSON — which the except-json.JSONDecodeError below then
            # silently swallowed as "no tool calls", every single time.
            # Raised well above realistic single-file sizes while still
            # respecting the caller's own max_tokens as the outer bound.
            base_url, prompt, min(max_tokens, 4096), temperature,
            grammar=PLAN_AND_DECISION_GRAMMAR,
        )
    except requests.exceptions.RequestException as e:
        print(f"  [plan+decision call failed: {e}]")
        return "", []
...
# Grammar-constrains classify_task_complexity's output to exactly one of
# these three words. Prompt wording alone ("reply with exactly one word")
# isn't reliable on a small model — it commonly hedges or adds a clause
# even with thinking mode off — and a naive substring match against that
# free-form text is what let hedge phrases like "moderately complex" or
# any other stray mention of "moderate" silently win the match every
# time, regardless of the actual answer. Constraining the grammar makes
# any output other than the three bare words structurally impossible,
# the same guarantee get_plan_and_decision already has.
CLASSIFY_GRAMMAR = 'root ::= "simple" | "moderate" | "complex"'
...
def _condense_tool_activity(new_messages):
    """Extracts this turn's tool calls and their results into one
    compact, labeled block for folding into the persistent cross-turn
    conversation — organized and bounded, rather than either losing
    tool context entirely between turns (only the model's own
    paraphrase of what a tool returned would survive, which may drop
    or round details a follow-up question needs), or dumping the full
    raw per-round scaffolding (subtask push/pop bookkeeping, invariant
    injections, reflection notes) that the next turn doesn't need and
    that would make cross-turn context grow unboundedly over a long
    session. Reuses the '(used X)' / 'Tool result from X: ...' pairs
    _short_result already produces in `working` — no new truncation
    logic, same size bound as what's already used within a turn.
    new_messages should be just the slice of working/messages added
    during the turn(s) being folded in, not the whole history.
    Returns "" if there's no tool activity to report.
    """
    lines = []
    i = 0
    while i < len(new_messages):
        ...
        if (m.get("role") == "assistant" and isinstance(content, str)
                and content.startswith("(used ") and content.endswith(")")):
            result_line = None
            if i + 1 < len(new_messages):
                nxt = new_messages[i + 1]
                nxt_content = nxt.get("content")
                if (nxt.get("role") == "user" and isinstance(nxt_content, str)
                        and nxt_content.startswith("Tool result from ")):
                    result_line = nxt_content
                    i += 1  # consume the paired result too, don't double-count it
            lines.append(f"- {result_line}" if result_line else f"- {content}")
        i += 1
    if not lines:
        return ""
    return "[Tool activity, for reference in follow-ups:\n" + "\n".join(lines) + "\n]"
...
def compact_context(base_url, working, keep_last=2, max_tokens=200, temperature=0.0):
    ...
    try:
        msg = chat_completion(base_url, digest_prompt, max_tokens, temperature, disable_thinking=True)
...
def reflect_on_progress(base_url, working, original_goal, max_tokens, temperature):
    ...
    try:
        msg = chat_completion(base_url, reflect_prompt, min(max_tokens, 60), 0.0, disable_thinking=True)
        text = (msg.get("content") or "").strip()
    except requests.exceptions.RequestException:
        return True, ""  # if the check itself fails, don't block progress on it

    # Whole-first-word match for the same reason as check_decomposable
    # and detect_contradiction — startswith("YES") alone would also
    # happen to match "Yesterday..." at the start of a reply.
    first_word_match = re.match(r'\s*([A-Za-z]+)', text)
    on_track = bool(first_word_match) and first_word_match.group(1).upper() == "YES"
...
def run_turn_with_tools(base_url, messages, max_tokens, temperature, console, original_goal=None, complexity_override=None):
    ...
    if complexity_override:
        ...
        #
        # Every saved playbook entry currently comes from /auto mode
        # (save_playbook is only ever called with complexity="autonomous"
        # — see main()'s /auto handling), so a match here could otherwise
        # hand an ordinary chat turn the reserved "autonomous" budget
        # (60 rounds / 1200s) purely because its wording resembled a past
        # /auto task. That's exactly the leak the "reserved... never
        # chosen by classify_task_complexity itself" comment on
        # COMPLEXITY_BUDGETS is meant to prevent — it just wasn't
        # actually enforced on this path. Reject an "autonomous" match
        # here and classify fresh instead, same as no match at all.
        playbook_match = find_playbook_match(original_goal) if original_goal else None
        if playbook_match and playbook_match[0] == "autonomous":
            playbook_match = None
        if playbook_match:
    ...
    for round_i in range(max_rounds):
        ...
        concurrent_batch = []
        # New improvement 3 fix: track success/failure per TOOL for this
        # round only, and reconcile into the persistent streak once the
        # round's dispatch (sequential + concurrent) is fully done —
        # rather than updating tool_failure_streak inline as each call
        # completes. Updating inline meant a single round batching 2
        # independent calls to the same tool (e.g. two web_searches,
        # exactly the batching improvement 7 encourages) that both
        # happened to fail counted as TWO consecutive failures against
        # TOOL_FAILURE_ESCALATION_THRESHOLD=2 and escalated after only
        # one real attempt — not the repeated-attempts-over-time signal
        # this was meant to catch.
        tools_touched_this_round = {}  # tool_name -> True if any call to it succeeded this round
        for call in calls:
            ...
            # that clearly isn't working. Reconciled into the actual
            # persistent streak once per round, below.
            if _is_tool_error(tool_name, result):
                tools_touched_this_round.setdefault(tool_name, False)
            else:
                tools_touched_this_round[tool_name] = True
        ...
        # Reconcile this round's per-tool outcomes into the persistent
        # streak exactly once per tool, regardless of how many individual
        # calls to it happened this round (sequential + concurrent
        # combined) — see the note above at tools_touched_this_round.
        for tool_name, any_success in tools_touched_this_round.items():
            if any_success:
...
def self_critique(base_url, working_messages, draft_answer, max_tokens, temperature, console):
    ...
    try:
        # Pinned to 0.0 rather than the passed-through chat temperature —
        # every other verification helper (classify, decompose-check,
        # contradiction-check, reflect, final-goal-check) already does
        # this for the same reason: this is meant to be a quick, cheap
        # yes/no-shaped check ("reply with exactly: OK"), and at a
        # creative temperature the model is far more likely to ramble
        # into a full rewritten answer using the whole max_tokens budget
        # even when the original was already fine, instead of the short
        # "OK" this is designed around.
        msg = chat_completion(base_url, critique_prompt, max_tokens, 0.0, disable_thinking=True)
        verdict = (msg.get("content") or "").strip()
    except requests.exceptions.RequestException:
        return draft_answer
...
def final_goal_verification(base_url, final_answer, original_goal, max_tokens, temperature, console):
    ...
    try:
        msg = chat_completion(base_url, check_prompt, min(max_tokens, 80), 0.0, disable_thinking=True)
        verdict = (msg.get("content") or "").strip()
    except requests.exceptions.RequestException:
        return final_answer
...
def run_selftest(base_url):
    ...
    def t_chat():
        msg = chat_completion(base_url, [{"role": "user", "content": "Say OK."}], 20, 0.0, disable_thinking=True)
    ...
    def t_chat_template_sanity():
        ...
        msg = chat_completion(base_url, [{"role": "user", "content": "Say hello in one word."}], 20, 0.0, disable_thinking=True)
    ...
    def t_grammar():
        grammar = build_plan_and_decision_grammar(TOOLS)
...
def stream_and_finalize(base_url, working, messages, max_tokens, temperature, ctx_size, console, tools_ran, original_goal=None, complexity="moderate", tool_activity_note=""):
    ...
    `tool_activity_note` (see _condense_tool_activity): if the caller
    built one, it's folded into the message actually appended to
    `messages` — not into the returned/displayed full_reply, since the
    user already watched the tool calls happen live during the round
    loop and doesn't need them repeated in the visible answer — so a
    follow-up turn's model can see what a tool actually returned, not
    just the model's own paraphrase of it."""
    ...
    history_content = f"{tool_activity_note}\n\n{full_reply}" if tool_activity_note else full_reply
...
def main():
    ...
    def generate_reply():
        ...
        tool_activity_note = _condense_tool_activity(working[pre_len:]) if tools_ran else ""
        full_reply, stats = stream_and_finalize(
            ...
            tool_activity_note=tool_activity_note,
        )
        if stats:
            last_stats = stats
